  ...
  const processPendingPaidSyncQueue = useCallback(async (): Promise<void> => {
    ...
    try {
      ...
      const applySnapshotForCurrentJob = (state: AppState, source: string): boolean => {
        ...
        const refreshServerDraftForCurrentJob = async (source: string): Promise<SaleDraft | undefined> => {
          const stateRefreshStartedAt = performance.now();
          const refreshedState = await fetchStateSnapshotControlled(currentJob.draftId);
          applySnapshotForCurrentJob(refreshedState, source);
          recordStateRefreshMs(performance.now() - stateRefreshStartedAt);
          return saleDraftsRef.current.find((entry) => entry.id === currentJob.draftId);
        };

        let draftReadyForConfirm = currentServerDraft;
        if (!draftReadyForConfirm || draftReadyForConfirm.status === 'DRAFT') {
          try {
            draftReadyForConfirm = await refreshServerDraftForCurrentJob(
              'pending_paid_preconfirm_status_check'
            );
          } catch {
            // best-effort: fallback to local snapshot validation below
          }
        }

        if (draftReadyForConfirm?.status === 'PAID' || draftReadyForConfirm?.status === 'CANCELLED') {
          setDraftLifecycleStage(
            currentJob.draftId,
            draftReadyForConfirm.status === 'CANCELLED' ? 'CANCELLED' : 'PAID',
            {
              reason: 'preconfirm_already_terminal',
              bumpEpoch: false,
            }
          );
          completePaymentFlowTelemetry(currentJob.draftId, {
        ...
        if (!draftReadyForConfirm || draftReadyForConfirm.status !== 'PENDING_PAYMENT') {
          const finalizeVerificationErrorSink: RunCommandErrorSink = {};
          const finalizeVerificationStartedAt = performance.now();
          let finalizedByVerification = false;
          try {
            finalizedByVerification = await handleSavePaymentMethod(currentJob.snapshot, {
              trackPendingState: false,
              silentSavedNotification: true,
              silentErrorNotification: true,
              errorSink: finalizeVerificationErrorSink,
              preferAsyncFinalize: false,
              failFastOnVersionConflict: false,
              skipSnapshotApplyOnTerminalFlow: true,
              onSnapshotAppliedMs: recordSnapshotApplyMs,
            ...
            markPaymentFlowTelemetryStageDuration(
              ...
              currentJob.id,
              'finalizeMs',
              performance.now() - finalizeVerificationStartedAt
            );
          }

          if (!finalizedByVerification) {
            await markJobAsFailed('Servidor ainda não confirmou finalização para pagamento.', {
              message:
                finalizeVerificationErrorSink.message ||
                'TERMINAL_FINALIZE_NOT_PERSISTED: finalize não confirmado pelo servidor.',
              retryable: false,
              statusCode: finalizeVerificationErrorSink.statusCode ?? 409,
            });
            return;
          }
          ...
          try {
            draftReadyForConfirm = await refreshServerDraftForCurrentJob(
              'pending_paid_preconfirm_verify_finalize'
            );
          } catch {
            // best-effort: fallback to state check below
          }

          if (!draftReadyForConfirm || draftReadyForConfirm.status !== 'PENDING_PAYMENT') {
            await markJobAsFailed('Servidor não refletiu finalização após verificação.', {
              message:
                'TERMINAL_FINALIZE_NOT_PERSISTED: draft não ficou em PENDING_PAYMENT após finalização.',
              retryable: false,
              statusCode: 409,
            });
            return;
